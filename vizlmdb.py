...
a =  lmdb.open('dataset/DocTamperV1-TrainingSet',readonly=True,lock=False,readahead=False,meminit=False)
# index = args.i
index = 0
...
with a.begin(write=False) as txn:
    ...
    buf.seek(0)
    im = Image.open(buf)
    im.save('visualization/original.jpg')
    ...
    if mask.max()==1:
        mask=mask*255
    cv2.imwrite('visualization/mask_fact.jpg',mask)
